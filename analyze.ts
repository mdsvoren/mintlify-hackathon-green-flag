...
export async function analyzeFeatureFlags(octokit: Octokit, anthropicClient: ClaudeSonnetClient, repository: string, branch: string, owner: string) {
  try {
    // Fetch feature flags
    ...
    const recentFeatureFlags = featureFlags.filter(flag => flag.change_date && new Date(flag.change_date) > twoMonthsAgo);

    console.log(`Number of feature flags newer than 2 months: ${recentFeatureFlags.length}`);
    ...
    console.log(`Number of FileWithRange objects created: ${fileWithRanges.length}`);
